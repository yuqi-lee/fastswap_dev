...
void rswap_setup_message_wr(struct rdma_session_context *rdma_session)
{
	...
	rdma_session->rdma_recv_req.recv_sgl.lkey =
		...
		//rdma_session->rdma_recv_req.recv_dma_mr->lkey;
...
int rswap_setup_buffers(struct rdma_session_context *rdma_session)
{
	...
	/*
	rdma_session->rdma_recv_req.recv_dma_mr = ib_get_dma_mr(rdma_session->rdma_dev->pd,
    	IB_ACCESS_LOCAL_WRITE | IB_ACCESS_REMOTE_WRITE | IB_ACCESS_REMOTE_READ);
	if (IS_ERR(rdma_session->rdma_recv_req.recv_dma_mr)) {
    	pr_err("Failed to get DMA MR\n");
    	return PTR_ERR(rdma_session->rdma_recv_req.recv_dma_mr);
	}*/
	...
	if (ib_dma_mapping_error(rdma_session->rdma_dev->dev, 
                       rdma_session->rdma_recv_req.recv_dma_addr)) {
    	pr_err("recv DMA mapping failed for recv_buf\n");
    	return -ENOMEM;
	}
	...
	if (ib_dma_mapping_error(rdma_session->rdma_dev->dev, 
                       rdma_session->rdma_send_req.send_dma_addr)) {
    	pr_err("send DMA mapping failed for send_buf\n");
    	return -ENOMEM;
	}
	...
	pr_info("%s, local key is: 0x%llx \n", __func__, (unsigned long long)rdma_session->rdma_dev->dev->local_dma_lkey);
