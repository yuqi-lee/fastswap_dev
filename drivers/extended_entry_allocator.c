...
#define ALLOCATOR "/dev/shm/allocator_page_queue"
#define DEALLOCATOR "/dev/shm/deallocator_page_queue"

struct allocator_page_queue *queue_allocator = NULL;
EXPORT_SYMBOL(queue_allocator);

struct deallocator_page_queue *queue_deallocator = NULL;
EXPORT_SYMBOL(queue_deallocator);
...
int allocator_page_queue_init(void) {
    ...
    ret = kern_path(ALLOCATOR, LOOKUP_FOLLOW, &path_);
    if (ret != 0) {
        // handle error
...
int deallocator_page_queue_init(void) {
    ...
    ret = kern_path(DEALLOCATOR, LOOKUP_FOLLOW, &path_);
    if (ret != 0) {
        // handle error
...
u64 get_length_allocator(void) {
    u64 begin = atomic64_read(&queue_allocator->begin);
    u64 end = atomic64_read(&queue_allocator->end);
    if (begin == end) {
        return 0;
    }
    if (end > begin) {
...
u64 get_length_deallocator(void) {
    u64 begin = atomic64_read(&queue_deallocator->begin);
    u64 end = atomic64_read(&queue_deallocator->end);
    if (begin == end) {
        return 0;
    }
    if (end > begin) {
...
int push_queue_deallocator(u64 page_addr) {
    int ret = 0;
    u64 prev_end = atomic64_read(&queue_deallocator->end);
